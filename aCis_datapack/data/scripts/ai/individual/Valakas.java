...
{
	private static final L2BossZone _valakasLair = GrandBossManager.getInstance().getZoneById(110010);
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		if (npc != null)
		{
			if (event.equalsIgnoreCase("beginning"))
			{
				...
				// Sound + socialAction.
				for (L2PcInstance plyr : _valakasLair.getKnownTypeInside(L2PcInstance.class))
				{
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet)
	{
		if (!Config.FWA_FIXTIMEPATTERNOFVALAKAS.isEmpty())
		{
			...
			startQuestTimer("valakas_unlock", getRespawnInterval(), null, null, false);
	...
	private static long getRespawnInterval()
	{
		...
		{
			throw new RuntimeException("Invalid respawn data \"" + Config.FWA_FIXTIMEPATTERNOFVALAKAS + "\" in " + Valakas.class.getSimpleName(), ipe);
		}
	}
