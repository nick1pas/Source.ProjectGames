...
{
    private static final L2BossZone _zakenLair = GrandBossManager.getInstance().getZoneById(110000);
    ...
    public Zaken(String name, String descr)
    {
        ...
        ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(new Runnable()
        {
        	...
        	public void run()
        	{
        		try
        		{
        			if (GetTimeHour() == 0 && status == ALIVE)
        			{
        				_log.info("Zaken door id 21240006 opened, game time 00.00.");
        				DoorTable.getInstance().getDoor(21240006).openMe();
        				Broadcast.gameAnnounceToOnlinePlayers("Devil's Isle door is now opened for 2 minutes!");
        				ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
        				{
        					...
        					public void run()
        					{
        						try
        						{
        							Broadcast.gameAnnounceToOnlinePlayers("Devil's Isle door is now closed!");
    ...
    public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet)
    {
		if (!Config.FWA_FIXTIMEPATTERNOFZAKEN.isEmpty())
		{
			...
			if (npcId == ZAKEN)
			{
				...
				startQuestTimer("zaken_unlock", getRespawnInterval(), null, null, false);
	...
	private static long getRespawnInterval()
	{
		...
		{
			throw new RuntimeException("Invalid respawn data \"" + Config.FWA_FIXTIMEPATTERNOFZAKEN + "\" in " + Zaken.class.getSimpleName(), ipe);
		}
	}
