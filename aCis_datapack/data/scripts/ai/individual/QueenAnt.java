...
public class QueenAnt extends AbstractNpcAI
{
		...
		if (!Config.FWA_FIXTIMEPATTERNOFQA.isEmpty())
		{
			...
			if (GrandBossManager.getInstance().getBossStatus(QUEEN) == ALIVE)
			{
				...
				if (npcId == QUEEN)
				{
					...
					startQuestTimer("queen_unlock", getRespawnInterval(), null, null, false);
	...
	private static long getRespawnInterval()
	{
		...
		{
			throw new RuntimeException("Invalid respawn data \"" + Config.FWA_FIXTIMEPATTERNOFQA + "\" in " + QueenAnt.class.getSimpleName(), ipe);
		}
	}
