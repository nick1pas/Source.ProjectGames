...
public class Baium extends AbstractNpcAI
{
		...
		if (npc != null && npc.getNpcId() == LIVE_BAIUM)
		{
			...
			else if (event.equalsIgnoreCase("sacrifice_waker"))
			{
				if (player != null)
				{
					...
					{
						_baiumLair.allowPlayerEntry(player, 10);
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet)
	{
		if (!Config.FWA_FIXTIMEPATTERNOFBAIUM.isEmpty())
		{
			...
			startQuestTimer("baium_unlock", getRespawnInterval(), null, null, false);
	...
	private static long getRespawnInterval()
	{
		...
		{
			throw new RuntimeException("Invalid respawn data \"" + Config.FWA_FIXTIMEPATTERNOFBAIUM + "\" in " + Baium.class.getSimpleName(), ipe);
		}
	}
