...
public class L2BufferInstance extends L2NpcInstance
{
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		else if (currentCommand.equalsIgnoreCase("paybuff"))
		{
			...
			if (buffid != 0 && player.destroyItemByItemId("buff",9501 ,buffPrice, player.getCurrentFolkNPC(), true))
			{
				player.broadcastPacket(new MagicSkillUse(this, player, buffid, bufflevel, 5, 0));
				player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_FEEL_S1_EFFECT).addSkillName(buffid, bufflevel));
				SkillTable.getInstance().getInfo(buffid, bufflevel).getEffects(this, player);
	...
	private void showGiveBuffsWindow(L2PcInstance player)
	{
		...
		else
		{
			for (Map.Entry<String, ArrayList<Integer>> scheme : schemes.entrySet())
			{
				...
				StringUtil.append(sb, "<a action=\"bypass npc_%objectId%_givebuffs ", scheme.getKey(), "\">Use on Me</a>&nbsp;|&nbsp;");
				StringUtil.append(sb, "<a action=\"bypass npc_%objectId%_givebuffs ", scheme.getKey(), " pet\">Use on Pet</a>&nbsp;|&nbsp;");
