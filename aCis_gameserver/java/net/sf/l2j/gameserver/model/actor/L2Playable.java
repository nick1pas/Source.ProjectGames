...
public abstract class L2Playable extends L2Character
{
	...
	public boolean doDie(L2Character killer)
	{
		...
		else if (isInsideZone(ZoneId.BOSS_AREA) || isInsideZone(ZoneId.FLAG_AREA_SELF) || isInsideZone(ZoneId.NO_ZERG))
		{
			if (Config.CHAOTIC_LEAVE_BUFFS_ON_DIE)
			{
				stopAllEffectsExceptThoseThatLastThroughDeath();

				if (actingPlayer.getClan() != null)
					Broadcast.QuakeAnnounce(getName() +" of the clan" + actingPlayer.getClan().getName() + " had his ass wiped and get self!");
				else
					Broadcast.QuakeAnnounce(getName() +" had his ass wiped and get self!");
			}
		}
		else if (Config.LEAVE_BUFFS_ON_DIE || !(TvTConfig.TVT_EVENT_EFFECTS_REMOVAL == 2))
			stopAllEffectsExceptThoseThatLastThroughDeath();
