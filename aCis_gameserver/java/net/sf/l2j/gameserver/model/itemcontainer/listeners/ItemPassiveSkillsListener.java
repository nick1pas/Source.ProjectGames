...
public class ItemPassiveSkillsListener implements OnEquipListener
{
	...
	public void onEquip(int slot, ItemInstance item, L2Playable actor)
	{
		...
		if (it instanceof Weapon)
		{
			...
			// Add skills bestowed from +22 Custom
			if (item.getEnchantLevel() >= 22)
			{
	...
	public void onUnequip(int slot, ItemInstance item, L2Playable actor)
	{
		...
		if (it instanceof Weapon)
		{
			...
			// Remove skills bestowed from +22 Custom
			if (item.getEnchantLevel() >= 22)
			{
				final L2Skill enchant22Skill = ((Weapon) it).getEnchant22Skill();
				if (enchant22Skill != null)
				{
					player.removeSkill(enchant22Skill, false, enchant22Skill.isPassive());
					update = true;
				}
			}
		}
