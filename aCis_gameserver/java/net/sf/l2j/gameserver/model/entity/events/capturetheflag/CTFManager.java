...
public class CTFManager
{
	...
	public void startReg()
	{
		...
		{
			Broadcast.gameAnnounceToOnlinePlayers("CTF Event: Event was cancelled.");
		...
		{
			Broadcast.gameAnnounceToOnlinePlayers("CTF Event: Joinable in " + CTFConfig.CTF_NPC_LOC_NAME + "!");
			
			if (Config.ALLOW_EVENT_COMMANDS)
			     Broadcast.gameAnnounceToOnlinePlayers("CTF Event: Command: .ctfjoin / .ctfleave / .ctfinfo");
	...
	public void startEvent()
	{
		...
		{
			Broadcast.gameAnnounceToOnlinePlayers("CTF Event: Event cancelled due to lack of Participation.");
	...
	class CTFStartTask implements Runnable
	{
		...
		private void announce(long time)
		{
			if ((time >= 3600) && ((time % 3600) == 0))
			{
				...
				{
					Broadcast.gameAnnounceToOnlinePlayers("CTF Event: " + (time / 60 / 60) + " hour(s) until registration is closed!");
				}
			...
			else if (time >= 60)
			{
				if (CTFEvent.isParticipating())
				{
					Broadcast.gameAnnounceToOnlinePlayers("CTF Event: " + (time / 60) + " minute(s) until registration is closed!");
				}
				else if (CTFEvent.isStarted())
				{
			...
			else
			{
				if (CTFEvent.isParticipating())
				{
					Broadcast.gameAnnounceToOnlinePlayers("CTF Event: " + time + " second(s) until registration is closed!");
				}
				else if (CTFEvent.isStarted())
				{
