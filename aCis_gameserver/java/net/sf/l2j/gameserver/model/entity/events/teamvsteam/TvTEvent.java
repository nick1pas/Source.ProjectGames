		...
		for (L2PcInstance playerInstance : team.getParticipatedPlayers().values())
		{
			...
			for (int[] reward : TvTConfig.TVT_EVENT_REWARDS)
			{
				...
				if (playerInstance.isVip())
				{
					...
					{
						inv.addItem("Team vs Team:", reward[0], reward[1] * Config.VIP_DROP_RATE, playerInstance, playerInstance);
					...
					else
					{
						...
						{
							inv.addItem("Team vs Team:", reward[0], 1 * Config.VIP_DROP_RATE, playerInstance, playerInstance);
							systemMessage = SystemMessage.getSystemMessage(SystemMessageId.EARNED_ITEM_S1);
							systemMessage.addItemName(reward[0]);
							playerInstance.sendPacket(systemMessage);
						}
