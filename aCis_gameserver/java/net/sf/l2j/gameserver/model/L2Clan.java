...
public class L2Clan
{
	...
	public void setNewLeader(L2ClanMember member)
	{
        ...
        ItemInstance crown = exLeader.getInventory().getItemByItemId(6841);
        if (crown != null)
        {
            if (crown.isEquipped())
                exLeader.getInventory().unEquipItemInSlot(crown.getLocationSlot());
            
            exLeader.destroyItemByItemId("Crown", 6841, 1, exLeader, true);
        }
		...
		for (Siege siege : SiegeManager.getSieges())
		{
			...
			for (Castle castle : CastleManager.getInstance().getCastles())
			{
				if (siege.checkIsAttacker(this))
				{
					...
					player.setSiegeSide(castle.getCastleId());
				}
