...
public final class Weapon extends Item
{
	...
	private SkillHolder _enchant22Skill = null; // skill that activates when item is enchanted +22 (Custom Feature)
	...
	public Weapon(StatsSet set)
	{
		...
		skill = set.getString("enchant22_skill", null);
		if (skill != null)
		{
			String[] info = skill.split("-");
			...
			if (info != null && info.length == 2)
			{
				...
				{
					// Incorrect syntax, dont add new skill
					_log.info(StringUtil.concat("> Couldnt parse ", skill, " in weapon enchant +22 skills! item ", toString()));
				}
				if (id > 0 && level > 0)
					_enchant22Skill = new SkillHolder(id, level);
			}
		}
	...
	/**
	 * @return The skill player obtains when he equiped weapon +22 or more (Custom Feature)
	 */
	public L2Skill getEnchant22Skill()
	{
		if (_enchant22Skill == null)
			return null;
		
		return _enchant22Skill.getSkill();
	}
