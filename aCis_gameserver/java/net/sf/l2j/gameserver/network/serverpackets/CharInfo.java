...
public class CharInfo extends L2GameServerPacket
{
	...
	protected final void writeImpl()
	{
		...
		if (_activeChar.getAppearance().getInvisible())
		{
			...
			if (tmp != null)
			{
				if (_activeChar.isArenaProtection() && party != null && !tmp.isArenaAttack() && tmp.isInArenaEvent() && party.getPartyMembers().contains(tmp))
					tournament_pt = true;
				else if (_activeChar.isArenaProtection() && tmp.isArenaAttack() && !tmp.inObserverMode())
					tournament_start = true;
				else if ((_activeChar.isInsideZone(ZoneId.TOURNAMENT) && !_activeChar.isGM() && !_activeChar.isOlympiadProtection()) && tmp.isInsideZone(ZoneId.TOURNAMENT) && !tmp.inObserverMode() && !tmp.isOlympiadProtection())
		...
		if (gmSeeInvis || tournament_pt || tournament_start || Olympiad_start || Olympiad_observer || tournament_zone)
			writeC(0);
		else
