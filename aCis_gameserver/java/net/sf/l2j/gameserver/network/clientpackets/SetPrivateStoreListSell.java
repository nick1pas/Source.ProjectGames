...
public final class SetPrivateStoreListSell extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (AttackStanceTaskManager.getInstance().get(player) || (player.isCastingNow() || player.isCastingSimultaneouslyNow()) || player.isInDuel() || player.isArenaProtection())
		{
			player.sendPacket(SystemMessageId.YOU_ARE_NOT_AUTHORIZED_TO_DO_THAT);
			player.sendPacket(new PrivateStoreManageListSell(player, _packageSale));
			return;
		...
		if (Config.PRIVATE_STORE_NEED_LEVELS)
		{
			if (player.getLevel() <= Config.MIN_LEVEL_TO_USE_STORE)
			{
				player.sendMessage("You must be level " + Config.MIN_LEVEL_TO_USE_STORE + " or above to open a private store.");
				player.setPrivateStoreType(L2PcInstance.STORE_PRIVATE_NONE);
				player.broadcastUserInfo();
				return;
			}
		...
		if (Config.PRIVATE_STORE_NEED_PVPS)
		{
