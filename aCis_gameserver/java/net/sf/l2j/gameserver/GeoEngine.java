...
public class GeoEngine extends GeoData
{
	...
	/**
	 * @see net.sf.l2j.gameserver.GeoData#getSpawnHeight(int, int, int, int, L2Spawn)
	 */
	@Override
	...
	public boolean canSeeTarget(L2Object cha, L2Object target)
	{
		...
		// and some layers can be very close...) do not change this without changing the LOS code.
		...
		// If this is going to be improved, use e.g. ((L2Character)cha).getTemplate().collisionHeight
	...
	public void addGeoDataBug(L2PcInstance gm, String comment)
	{
		...
		int cx = getCell(gx);
		int cy = getCell(gy);
		int rx = (gx >> 11) + L2World.WORLD_X_MIN;
		int ry = (gy >> 11) + L2World.WORLD_X_MAX;
	...
	private static void nInitGeodata()
	{
		...
		try
		{
			...
			_log.info("GeoEngine: Loaded " + count + " geofiles.");
		}
		catch (Exception e)
		{
			_log.log(Level.WARNING, "", e);
	...
	public static boolean loadGeodataFile(byte rx, byte ry)
	{
		...
		if (Config.DEVELOPER)
			_log.info("GeoEngine: Loading area " + rx + "_" + ry + " -> region offset: " + regionoffset);
		
		File Geo = new File(fname);
		int size, index = 0, block = 0, flor = 0;
		try (FileChannel roChannel = new RandomAccessFile(Geo, "r").getChannel())
		{
			size = (int) roChannel.size();
			MappedByteBuffer geo;
			
			// Force O/S to load this buffer's content into physical memory.
			// It is not guaranteed, because the underlying operating system may have paged out some of the buffer's data
	...
	private static short getRegionOffset(int x, int y)
	{
		int rx = x >> 11; // =/(256 * 8)
		int ry = y >> 11;
		return (short) (((rx + L2World.WORLD_X_MIN) << 5) + (ry + L2World.WORLD_Y_MIN));
	}
