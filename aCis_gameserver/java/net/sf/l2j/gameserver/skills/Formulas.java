...
public final class Formulas
{
	...
	public static double calcBlowDamage(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean ss)
	{
		...
		{
			final Armor armor = ((L2PcInstance) target).getActiveChestArmorItem();
			if (armor != null)
			{
 		...
 		if (Config.ENABLE_CLASS_DAMAGES && attacker instanceof L2PcInstance && target instanceof L2PcInstance)
 		{
 			if (((L2PcInstance) attacker).isInOlympiadMode() && ((L2PcInstance) target).isInOlympiadMode())
 			{
 				...
 				{
 					damage = damage*OlyClassDamageManager.getDamageMultiplier((L2PcInstance) attacker, (L2PcInstance) target);
 				}
	...
	public static final double calcPhysDam(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean crit, boolean ss)
	{
		...
		if (target instanceof L2PcInstance && weapon != null && weapon.getItemType() == WeaponType.DAGGER && skill != null)
		{
			final Armor armor = ((L2PcInstance) target).getActiveChestArmorItem();
			if (armor != null)
			{
		...
		if (target instanceof L2PcInstance && weapon != null && weapon.getItemType() == WeaponType.DUAL && skill != null)
		{
			final Armor armor = ((L2PcInstance) target).getActiveChestArmorItem();
			if (armor != null)
			{
				if (((L2PcInstance) target).isWearingHeavyArmor())
				{
					damage /= Config.ALT_DUAL_DMG_VS_HEAVY;
				}
				if (((L2PcInstance) target).isWearingLightArmor())
				{
					damage /= Config.ALT_DUAL_DMG_VS_LIGHT;
				}
				if (((L2PcInstance) target).isWearingMagicArmor())
				{
					damage /= Config.ALT_DUAL_DMG_VS_ROBE;
				}
			}
		}
	...
	public static final int calcPAtkSpd(L2Character attacker, L2Character target, double rate)
	{
		...
		return (int) (Config.ANTI_SS_BUG_2 / rate);
	}
