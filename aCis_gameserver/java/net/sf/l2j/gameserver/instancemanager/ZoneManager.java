...
import net.sf.l2j.gameserver.model.zone.type.L2InvasionZone;
...
public class ZoneManager
{
				...
				for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
				{
					if ("zone".equalsIgnoreCase(d.getNodeName()))
					{
						...
						for (Node cd = d.getFirstChild(); cd != null; cd = cd.getNextSibling())
						{
							...
							else if (temp instanceof L2ChangePvpZone)
							{
								if ("zoneName".equalsIgnoreCase(cd.getNodeName()))
								{
									attrs = cd.getAttributes();
									String name = attrs.getNamedItem("name").getNodeValue();
									((L2ChangePvpZone) temp).addName(name);
								}
							}
						}
						if (checkId(zoneId))
	...
	public final static L2ChaoticFreeZone getChaos(L2Character character)
	{
		if (character == null)
			return null;
		...
		for (L2ZoneType temp : ZoneManager.getInstance().getZones(character.getX(), character.getY(), character.getZ()))
		{
			if (temp instanceof L2ChaoticFreeZone && temp.isCharacterInZone(character))
				return ((L2ChaoticFreeZone) temp);
		}
		
		return null;
	}
	
	public final static L2ChaoticSelfZone getFlag(L2Character character)
	{
		if (character == null)
			return null;
		...
		for (L2ZoneType temp : ZoneManager.getInstance().getZones(character.getX(), character.getY(), character.getZ()))
		{
			if (temp instanceof L2ChaoticSelfZone && temp.isCharacterInZone(character))
				return ((L2ChaoticSelfZone) temp);
		}
	...
	public final static L2PartyFarmZone getParty(L2Character character)
	{
		...
		for (L2ZoneType temp : ZoneManager.getInstance().getZones(character.getX(), character.getY(), character.getZ()))
		{
			if (temp instanceof L2PartyFarmZone && temp.isCharacterInZone(character))
				return ((L2PartyFarmZone) temp);
		}
		
		return null;
	}
	
	public final static L2NoZergZone getZerg(L2Character character)
	{
		if (character == null)
			return null;
		...
		for (L2ZoneType temp : ZoneManager.getInstance().getZones(character.getX(), character.getY(), character.getZ()))
		{
			if (temp instanceof L2NoZergZone && temp.isCharacterInZone(character))
				return ((L2NoZergZone) temp);
		}
		
		return null;
	}
	
	public final static L2ChangePvpZone getChange(L2Character character)
	{
		if (character == null)
			return null;
		...
		for (L2ZoneType temp : ZoneManager.getInstance().getZones(character.getX(), character.getY(), character.getZ()))
		{
			if (temp instanceof L2ChangePvpZone && temp.isCharacterInZone(character))
				return ((L2ChangePvpZone) temp);
		}
		
		return null;
	}
	
	public final static L2InvasionZone getInvasionEvent(L2Character character)
	{
		if (character == null)
			return null;
		...
		for (L2ZoneType temp : ZoneManager.getInstance().getZones(character.getX(), character.getY(), character.getZ()))
		{
			if (temp instanceof L2InvasionZone && temp.isCharacterInZone(character))
				return ((L2InvasionZone) temp);
		}
