...
public final class PvpFlagTaskManager implements Runnable
{
	...
	public final void run()
	{
		...
		for (Map.Entry<L2PcInstance, Long> entry : _players.entrySet())
		{
			...
			if (player.isInsideZone(ZoneId.FLAG_AREA) || player.isInsideZone(ZoneId.FLAG_AREA_SELF) || player.isInsideZone(ZoneId.PARTY_FARM) || player.isInsideZone(ZoneId.NO_ZERG) || player.isInArenaEvent())
			{
				_players.remove(player);
				continue;
			}
