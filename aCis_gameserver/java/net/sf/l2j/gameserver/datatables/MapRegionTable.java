...
public class MapRegionTable
{
	...
	public Location getTeleToLocation(L2Character activeChar, TeleportWhereType teleportWhere)
	{
		if (activeChar instanceof L2PcInstance)
		{
			...
			// Checking if in Party Farm Zone
			L2PartyFarmZone party = ZoneManager.getParty(player);
			if (party != null)
				return party.getSpawnLoc();
			
			// Checking if in Party Zerg Zone
			L2NoZergZone zerg = ZoneManager.getZerg(player);
			if (zerg != null)
				return zerg.getSpawnLoc();

			// Checking if in Change PvP Zone
			L2ChangePvpZone changer = ZoneManager.getChange(player);
			if (changer != null)
				return changer.getSpawnLoc();
			
			// Checking if in Free Chaotic Zone
			L2ChaoticFreeZone chaos = ZoneManager.getChaos(player);
			if (chaos != null)
				return chaos.getSpawnLoc();
			
			// Checking if in Self Chaotic Zone
			L2ChaoticSelfZone flag = ZoneManager.getFlag(player);
			if (flag != null)
				return flag.getSpawnLoc();
			
			// Checking if in Self Chaotic Zone
			L2InvasionZone invasion = ZoneManager.getInvasionEvent(player);
			if (invasion != null)
				return invasion.getSpawnLoc();
