...
public class MultisellData
{
	 ...
	 */
	private ListContainer generateMultiSell(String listName, boolean inventoryOnly, L2PcInstance player, double taxRate)
	{
		int listId = listName.hashCode();
	...
	private void parse()
	{
		...
		if (Config.LOAD_CUSTOM_MULTISELLS)
		{
		...
		for (File f : files)
		{
			try
			{
				id = f.getName().replaceAll(".xml", "").hashCode();
		...
		_log.log(Level.INFO, "Multisell: Loaded " + _entries.size() + " files.");
	}
