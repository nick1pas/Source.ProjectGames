...
public class EnchantVipTable
{
	private static Logger _log = Logger.getLogger(EnchantVipTable.class.getName());
	
	private static final Map<Integer, L2EnchantScroll> _map = new HashMap<>();
	
	public static EnchantVipTable getInstance()
	{
		return SingletonHolder._instance;
	}
	
	protected EnchantVipTable()
	{
		try
		{
			File f = new File("./data/xml/enchant/vip_enchants.xml");
				...
				if ("list".equalsIgnoreCase(n.getNodeName()))
				{
					for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
					{
						if ("enchant".equalsIgnoreCase(d.getNodeName()))
						{
							...
							int backTo = Integer.valueOf(attrs.getNamedItem("backTo").getNodeValue());
							...
							_map.put(id, new L2EnchantScroll(grade_test, weapon, breaks, maintain, backTo, chance));
						}
					}
				}
			}
				
			_log.info("EnchantVipTable: Loaded Vip " + _map.size() + " enchants.");
		}
		catch (Exception e)
		{
			_log.warning("EnchantVipTable: Error while loading vip enchant table: " + e);
		}
	}
	...
	{
		protected static final EnchantVipTable _instance = new EnchantVipTable();
	}
}