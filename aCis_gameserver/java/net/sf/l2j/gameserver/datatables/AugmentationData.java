...
public class AugmentationData
{
	...
	private final void load()
	{
		...
		try
		{
			...
			File file = new File("./data/xml/augmentation/augmentation_skillmap.xml");
		...
		for (int i = 1; i < 5; i++)
		{
			try
			{
				File file = new File("./data/xml/augmentation/augmentation_stats" + i + ".xml");
				final Document doc = XMLDocumentFactory.getInstance().loadDocument(file);
		...
		switch (lifeStoneGrade)
		{
			case AbstractRefinePacket.GRADE_NONE:
				...
				if (Rnd.get(1, 100) <= Config.AUGMENTATION_NG_GLOW_CHANCE)
					generateGlow = true;
				if (!generateSkill && Rnd.get(1, 100) <= Config.AUGMENTATION_NG_BASESTAT_CHANCE)
			...
			case AbstractRefinePacket.GRADE_MID:
				...
				if (Rnd.get(1, 100) <= Config.AUGMENTATION_MID_GLOW_CHANCE)
					generateGlow = true;
				if (!generateSkill && Rnd.get(1, 100) <= Config.AUGMENTATION_MID_BASESTAT_CHANCE)
					stat34 = Rnd.get(BASESTAT_STR, BASESTAT_MEN);
				break;
			case AbstractRefinePacket.GRADE_HIGH:
				...
				if (Rnd.get(1, 100) <= Config.AUGMENTATION_HIGH_GLOW_CHANCE)
					generateGlow = true;
				if (!generateSkill && Rnd.get(1, 100) <= Config.AUGMENTATION_HIGH_BASESTAT_CHANCE)
					stat34 = Rnd.get(BASESTAT_STR, BASESTAT_MEN);
				break;
			case AbstractRefinePacket.GRADE_TOP:
				...
				if (Rnd.get(1, 100) <= Config.AUGMENTATION_TOP_GLOW_CHANCE)
					generateGlow = true;
				if (!generateSkill && Rnd.get(1, 100) <= Config.AUGMENTATION_TOP_BASESTAT_CHANCE)
					stat34 = Rnd.get(BASESTAT_STR, BASESTAT_MEN);
				break;
		}
