	...
	private static void setEnchant(L2PcInstance activeChar, int ench, int armorType)
	{
		...
		if (item != null && item.getLocationSlot() == armorType)
		{
			...
			if (item.isEquipped())
			{
				...
				// Skill bestowed by +22 Custom.
				else if (it instanceof Weapon)
				{
					// Old enchant was >= 22 and new is lower : we drop the skill.
					if (oldEnchant >= 22 && currentEnchant < 22)
					{
						final L2Skill enchant22Skill = ((Weapon) it).getEnchant22Skill();
						if (enchant22Skill != null)
						{
							player.removeSkill(enchant22Skill, false);
							player.sendSkillList();
						}
					}
					// Old enchant was < 22 and new is 22 or more : we add the skill.
					else if (oldEnchant < 22 && currentEnchant >= 22)
					{
						final L2Skill enchant22Skill = ((Weapon) it).getEnchant22Skill();
						if (enchant22Skill != null)
						{
							player.addSkill(enchant22Skill, false);
							player.sendSkillList();
						}
					}
				}
				...
				else if (it instanceof Armor)
				{
					...
					else if (oldEnchant < 22 && currentEnchant >= 22)
					{
						...
						{
							final ArmorSet armorSet = ArmorSetsTable.getInstance().getSet(chestItem.getItemId());
							if (armorSet != null && armorSet.isEnchanted22(player)) // has all parts of set enchanted to 22 or more
							{
								final int skillId = armorSet.getEnchant22skillId();
								if (skillId > 0)
								{
