...
public abstract class CombatAI extends FakePlayerAI
{
	...
	protected void tryAttackingUsingMageOffensiveSkill()
	{
		if (_fakePlayer.getTarget() != null)
		{
			...
			L2Skill skill = _fakePlayer.getKnownSkill(botSkill.getSkillId());
			if (skill != null)
				castSpell(skill);
		}
	}
	...
	protected int getShotId()
	{
		...
		if (playerLevel >= 76)
			return getShotType() == ShotType.SOULSHOT ? FakePlayerConfig.FAKE_PLAYER_SOULSHOT : FakePlayerConfig.FAKE_PLAYER_BLESSED_SOULSHOT;
	...
	protected int getArrowId()
	{
		...
		if (playerLevel >= 76)
			return FakePlayerConfig.FAKE_PLAYER_ARROW; // shining
	...
	protected void handleShots()
	{
		if (_fakePlayer.getInventory().getItemByItemId(getShotId()) != null) 
		{
			if(_fakePlayer.getInventory().getItemByItemId(getShotId()).getCount() <= 20) 
				_fakePlayer.getInventory().addItem("", getShotId(), 2500, _fakePlayer, null);			
		}
		else
			_fakePlayer.getInventory().addItem("", getShotId(), 2500, _fakePlayer, null);
	...
	protected L2Skill getRandomAvaiableFighterSpellForTarget() 
	{
		...
		L2Skill skill = _fakePlayer.getKnownSkill(spellsOrdered.get(skillIndex).getSkillId());
